...
* First-run setup overlay prompts you to enter your own APM URLs. No personal endpoints are pre-filled
* Endpoint config persists in localStorage across page refreshes
* Live proxy status badge shows green when the proxy is running, red with the start command when it is not
...
* Kibana URL is optional and is used to generate quick-links in the tool
...
* Search abandoned: Homepage, Search, Filter
* Power user: 5 steps including Homepage, Search, Filter, Product, Checkout
...
Check the boxes at the top to choose which endpoints receive the next event. Uncheck one to exclude it and re-check to include it again. You can see the result in the activity log: one status line per endpoint per event.
...
A 202 response from the APM intake API means the event was accepted for processing. Allow 10 to 15 seconds before refreshing Kibana.
...
| `rum-proxy.py` | CORS proxy, reads X-Target-Url header for generic routing |
